...
function getSSCDetails(annualGross: number, status: string): SSCDetails {
  ...
  if (status === "Exempt from paying NI/SSC") {
    return { weeklyGross, weeklySSC: 0, annualSSC: 0, rateDescription: "Exempt", logicDescription: "User is exempt from SSC." };
  }
  if (status === "Student (under 18 years old)") {
    const v = weeklyGross * 0.1;
    if (v < 4.38) { weeklySSC = v; rateDescription = "10% of Weekly Gross"; logicDescription = "Student rate (10%) is lower than the €4.38 cap."; }
    else { weeklySSC = 4.38; rateDescription = "€4.38 / week (Fixed Cap)"; logicDescription = "Student rate capped at €4.38 maximum."; }
  } else if (status === "Student (18 years old and over)") {
    const v = weeklyGross * 0.1;
    if (v < 7.94) { weeklySSC = v; rateDescription = "10% of Weekly Gross"; logicDescription = "Student rate (10%) is lower than the €7.94 cap."; }
    else { weeklySSC = 7.94; rateDescription = "€7.94 / week (Fixed Cap)"; logicDescription = "Student rate capped at €7.94 maximum."; }
  } else if (status === "Employed (under 18 years old)") {
    if (weeklyGross <= 221.78) { weeklySSC = 6.62; rateDescription = "€6.62 / week (Minimum)"; logicDescription = "Weekly income is below or equal to €221.78 (Minimum Rate applies)."; }
    else if (weeklyGross <= 544.28) { weeklySSC = weeklyGross * 0.1; rateDescription = "10% of Weekly Gross"; logicDescription = "Weekly income is between €221.79 and €544.28 (Standard 10% applies)."; }
    else { weeklySSC = 54.43; rateDescription = "€54.43 / week (Maximum)"; logicDescription = "Weekly income exceeds €544.28 (Maximum Cap applies)."; }
  } else if (status === "Employed (18 years old and over, born on or before 31 Dec 1961)") {
    if (weeklyGross <= 221.78) { weeklySSC = 22.18; rateDescription = "€22.18 / week (Minimum)"; logicDescription = "Weekly income is below or equal to €221.78 (Minimum Rate applies)."; }
    else if (weeklyGross <= 451.91) { weeklySSC = weeklyGross * 0.1; rateDescription = "10% of Weekly Gross"; logicDescription = "Weekly income is between €221.79 and €451.91 (Standard 10% applies)."; }
    else { weeklySSC = 45.19; rateDescription = "€45.19 / week (Maximum)"; logicDescription = "Weekly income exceeds €451.91 (Maximum Cap applies for born <= 1961)."; }
  } else {
    if (weeklyGross <= 221.78) { weeklySSC = 22.18; rateDescription = "€22.18 / week (Minimum)"; logicDescription = "Weekly income is below or equal to €221.78 (Minimum Rate applies)."; }
    else if (weeklyGross <= 544.28) { weeklySSC = weeklyGross * 0.1; rateDescription = "10% of Weekly Gross"; logicDescription = "Weekly income is between €221.79 and €544.28 (Standard 10% applies)."; }
    else { weeklySSC = 54.43; rateDescription = "€54.43 / week (Maximum)"; logicDescription = "Weekly income exceeds €544.28 (Maximum Cap applies)."; }
  }
  return { weeklyGross, weeklySSC, annualSSC: weeklySSC * 52, rateDescription, logicDescription };
}

function calculateTaxForYear(chargeableIncome: number, year: 2026 | 2027 | 2028, taxStatus: TaxStatusType): number {
  let tax = 0;
  const s = taxStatus;
  if (year === 2026) {
    if (s === "Married with One Child") {
      if (chargeableIncome <= 17500) tax = 0;
      else if (chargeableIncome <= 26500) tax = chargeableIncome * 0.15 - 2625;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 5275;
      else tax = chargeableIncome * 0.35 - 11275;
    } else if (s === "Married with Two or More Children") {
      if (chargeableIncome <= 22500) tax = 0;
      else if (chargeableIncome <= 32000) tax = chargeableIncome * 0.15 - 3375;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 6575;
      else tax = chargeableIncome * 0.35 - 12575;
    } else if (s === "Parent with One Child") {
      if (chargeableIncome <= 14500) tax = 0;
      else if (chargeableIncome <= 21000) tax = chargeableIncome * 0.15 - 2175;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 4275;
      else tax = chargeableIncome * 0.35 - 10270;
    } else if (s === "Parent with Two or More Children") {
      if (chargeableIncome <= 18500) tax = 0;
      else if (chargeableIncome <= 25500) tax = chargeableIncome * 0.15 - 2775;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 5325;
      else tax = chargeableIncome * 0.35 - 11325;
    }
  } else if (year === 2027) {
    if (s === "Married with One Child") {
      if (chargeableIncome <= 20000) tax = 0;
      else if (chargeableIncome <= 30000) tax = chargeableIncome * 0.15 - 3000;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 6000;
      else tax = chargeableIncome * 0.35 - 12000;
    } else if (s === "Married with Two or More Children") {
      if (chargeableIncome <= 30000) tax = 0;
      else if (chargeableIncome <= 41000) tax = chargeableIncome * 0.15 - 4500;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 8600;
      else tax = chargeableIncome * 0.35 - 14600;
    } else if (s === "Parent with One Child") {
      if (chargeableIncome <= 16000) tax = 0;
      else if (chargeableIncome <= 24500) tax = chargeableIncome * 0.15 - 2400;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 4850;
      else tax = chargeableIncome * 0.35 - 10850;
    } else if (s === "Parent with Two or More Children") {
      if (chargeableIncome <= 24000) tax = 0;
      else if (chargeableIncome <= 33500) tax = chargeableIncome * 0.15 - 3600;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 6950;
      else tax = chargeableIncome * 0.35 - 12950;
    }
  } else if (year === 2028) {
    if (s === "Married with One Child") {
      if (chargeableIncome <= 22500) tax = 0;
      else if (chargeableIncome <= 33500) tax = chargeableIncome * 0.15 - 3375;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 6725;
      else tax = chargeableIncome * 0.35 - 12725;
    } else if (s === "Married with Two or More Children") {
      if (chargeableIncome <= 37000) tax = 0;
      else if (chargeableIncome <= 50000) tax = chargeableIncome * 0.15 - 5550;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 10550;
      else tax = chargeableIncome * 0.35 - 16550;
    } else if (s === "Parent with One Child") {
      if (chargeableIncome <= 18000) tax = 0;
      else if (chargeableIncome <= 28000) tax = chargeableIncome * 0.15 - 2700;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 5500;
      else tax = chargeableIncome * 0.35 - 11500;
    } else if (s === "Parent with Two or More Children") {
      if (chargeableIncome <= 30000) tax = 0;
      else if (chargeableIncome <= 42000) tax = chargeableIncome * 0.15 - 4500;
      else if (chargeableIncome <= 60000) tax = chargeableIncome * 0.25 - 8700;
...
// Static lookup — defined once, never recreated
const TAX_TABLE_ROWS: Record<string, Record<number, string[][]>> = {
  "Married with One Child": {
    2026: [["0 – 17,500","0%","0"],["17,501 – 26,500","15%","2,625"],["26,501 – 60,000","25%","5,275"],["60,001+","35%","11,275"]],
    2027: [["0 – 20,000","0%","0"],["20,001 – 30,000","15%","3,000"],["30,001 – 60,000","25%","6,000"],["60,001+","35%","12,000"]],
    2028: [["0 – 22,500","0%","0"],["22,501 – 33,500","15%","3,375"],["33,501 – 60,000","25%","6,725"],["60,001+","35%","12,725"]],
  },
  "Married with Two or More Children": {
    2026: [["0 – 22,500","0%","0"],["22,501 – 32,000","15%","3,375"],["32,001 – 60,000","25%","6,575"],["60,001+","35%","12,575"]],
    2027: [["0 – 30,000","0%","0"],["30,001 – 41,000","15%","4,500"],["41,001 – 60,000","25%","8,600"],["60,001+","35%","14,600"]],
    2028: [["0 – 37,000","0%","0"],["37,001 – 50,000","15%","5,550"],["50,001 – 60,000","25%","10,550"],["60,001+","35%","16,550"]],
  },
  "Parent with One Child": {
    2026: [["0 – 14,500","0%","0"],["14,501 – 21,000","15%","2,175"],["21,001 – 60,000","25%","4,275"],["60,001+","35%","10,270"]],
    2027: [["0 – 16,000","0%","0"],["16,001 – 24,500","15%","2,400"],["24,501 – 60,000","25%","4,850"],["60,001+","35%","10,850"]],
    2028: [["0 – 18,000","0%","0"],["18,001 – 28,000","15%","2,700"],["28,001 – 60,000","25%","5,500"],["60,001+","35%","11,500"]],
  },
  "Parent with Two or More Children": {
    2026: [["0 – 18,500","0%","0"],["18,501 – 25,500","15%","2,775"],["25,501 – 60,000","25%","5,325"],["60,001+","35%","11,325"]],
    2027: [["0 – 24,000","0%","0"],["24,001 – 33,500","15%","3,600"],["33,501 – 60,000","25%","6,950"],["60,001+","35%","12,950"]],
    2028: [["0 – 30,000","0%","0"],["30,001 – 42,000","15%","4,500"],["42,001 – 60,000","25%","8,700"],["60,001+","35%","14,700"]],
  },
};
...
export default function MarriedParentTaxCalculator() {
  ...
  const results = useMemo((): FullCalculation & { sscDetails: SSCDetails } => {
    const zeroYear: YearCalculation = { taxableIncome: 0, mainTax: 0, partTimeTax: 0, totalTax: 0, netIncome: 0 };
    if (!form.grossSalary) {
      return {
        grossIncome: 0, ssc: 0, govBonus: 0,
        year2026: zeroYear, year2027: zeroYear, year2028: zeroYear,
        sscDetails: { weeklyGross: 0, weeklySSC: 0, annualSSC: 0, rateDescription: "-", logicDescription: "-" },
      };
    }
    ...
    const buildYear = (year: 2026 | 2027 | 2028): YearCalculation => {
      const mainTax = calculateTaxForYear(chargeableIncome, year, form.taxStatus);
      const totalTax = mainTax + partTimeTax;
      return { taxableIncome: chargeableIncome, mainTax, partTimeTax, totalTax, netIncome: totalGross + GOV_BONUS - totalTax - ssc };
    };
    return { grossIncome: totalGross, ssc, govBonus: GOV_BONUS, year2026: buildYear(2026), year2027: buildYear(2027), year2028: buildYear(2028), sscDetails };
  }, [form]);
...
function TaxTable({ year, status }: { year: 2026 | 2027 | 2028; status: TaxStatusType }) {
  const rows = TAX_TABLE_ROWS[status]?.[year] ?? [];
  return (
    <Table className="border text-xs dark:border-zinc-800">
      ...
      <TableBody>
        {rows.map((r) => (
          <TableRow key={r[0]} className="dark:border-zinc-800">
